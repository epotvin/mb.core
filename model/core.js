...
define(function(require, exports, module) {
    ...
    function main(options, imports, register) {
        ...
        var core_Model_version = new plugin.Attribute(model);
        var core_Model_dependencies = new plugin.Attribute(model);
        ...
        Object.defineProperties(plugin.Model.prototype, {
            ...
            version: {
                get: function() {
                    return this.get(core_Model_version);
                },
                set: function(version) {
                    this.set(core_Model_version, version);
                }
            },
            dependencies: {
                get: function() {
                    return this.get(core_Model_dependencies);
                },
                set: function(dependencies) {
                    this.set(core_Model_dependencies, dependencies);
                }
            }
        });
        ...
        plugin.getModel = function() {
            return model;
        };

        plugin.newInstance = function(model, clazz) {
            var element = null;
            if (clazz.is(core_Class)) {
                element = new plugin.Class(model);
            }
            else if (clazz.is(core_Attribute)) {
                element = new plugin.Attribute(model);
            }
            else if (clazz.is(core_Package)) {
                element = new plugin.Package(model);
            }
            else if (clazz.is(core_RootElement)) {
                element = new plugin.RootElement(model);
            }
            else {
                element = new plugin.Element(model);
            }
            return element;
        };

        plugin.newModel = function() {
            var newModel = new plugin.Model();
            newModel.instanceOf = core_Model;
            newModel.model = newModel;
            newModel.dependencies = [model];
            return newModel;
        };
